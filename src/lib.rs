        ...
        for _ in 0..number_of_clients {
            ...
            for _ in 0..number_of_structs {
                ...
                item.integrate(
                    ...
                    &mut ss,
                    StructPtr {
                        pivot: clock as usize,
                        id: ID { client, clock },
                    },
                );
                // add item to struct list
                // @todo try borow of index and generalize in ss
                let client_struct_list = ss
                    .structs
                    .entry(client)
                    .or_insert_with(|| UserStructList::with_capacity(number_of_structs as usize));
...
struct DocInner {
    ...
    update_handlers: RefCell<Vec<Weak<dyn UpdateObserver>>>,
}
...
impl DocInner {
    fn write_structs(&self, update_encoder: &mut encoding::UpdateEncoder, sv: &StateVector) {
        ...
        let mut structs: Vec<(&u32, &UserStructList)> = ss
            .structs
            .iter()
        ...
        for (client_id, client_structs) in structs.iter() {
            ...
            update_encoder
                .rest_encoder
                .write_var_u32(client_structs.integrated_len as u32);
...
impl Drop for Transaction {
    fn drop(&mut self) {
        let update_encoder = &mut encoding::UpdateEncoder::new();
        self.doc
            .write_structs(update_encoder, &self.start_state_vector);
...
impl Transaction {
    ...
    fn get_type_from_ptr(&self, ptr: &TypePtr) -> Rc<TypeInner> {
        match ptr {
            TypePtr::Named(name_ref) => self.doc.types.borrow()[*name_ref as usize].0.clone(),
        }
    }
    #[inline(always)]
    fn create_item(&mut self, pos: &ItemPosition, content: char) {
        let mut ss: std::cell::RefMut<'_, StructStore> = self.doc.ss.borrow_mut();
        let left = pos.after;
        ...
        let own_item_ptr = StructPtr { pivot, id };
...
impl StructStore {
    ...
    #[inline(always)]
    fn gen_item_ptr(&self, id: &ID) -> StructPtr {
        StructPtr {
            id: *id,
    ...
    fn get_item_mut(&mut self, ptr: &StructPtr) -> &mut Item {
        if ptr.id.client == self.client_id {
            unsafe { self.local_struct_list.list.get_unchecked_mut(ptr.pivot) }
        } else {
            unsafe {
...
impl Item {
    ...
    fn integrate(&self, tr: &Transaction, ss: &mut StructStore, own_item_ptr: StructPtr) {
        ...
        match self.left.as_ref() {
            ...
            None => {
                let parent_type: Rc<TypeInner> = tr.get_type_from_ptr(&self.parent);
